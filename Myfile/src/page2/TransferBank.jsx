...
export default function TransferBank() {const jwt =JSON.parse(localStorage.getItem('jwt'));
...
const mess =JSON.parse(localStorage.getItem('message'));
  ...
  const { register, handleSubmit, formState: { errors } } = useForm();
    const onSubmit = (data) => {
      ...
      swal({
        title: "Are you sure?",
        text: `Do you want to send ${amount} Birr to this Account Number: ${toAccountNumber}`,
...
async function login(){
 ...
 const options={
  ...
  const url ="http://192.168.1.27:8080/api/accounts/sendmoney"
  try{
